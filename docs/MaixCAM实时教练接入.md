...
3. 将 `maixcam/app.yaml`、`maixcam/main.py`、`maixcam/realtime_coach.py` 和 `maixcam/squat_logic.py` 复制到设备的 `/maixapp/apps/maixcoach/`。
...
0.3.0 起，每组开始先保持站直约半秒完成个人角度校准。计数使用 5 帧中位数和平滑状态确认；人体或关键点连续丢失时会取消未完成动作，避免重新入镜后误计。有效次数会在动作完成后立即上报，不再慢一拍。
...
首页“今日训练”区域提供 MaixCAM 设备状态和训练入口；设备卡不再常驻悬浮。用户主动点击后才打开“3 组 × 10 次”深蹲训练控制面板，训练结束后回到普通首页。网页负责组间 60 秒休息、下一组指令和浏览器语音播报；MaixCAM 负责姿态、计数和动作纠错。

语音播报会优先选择系统中评分最高的简体中文自然音色。动作鼓励、纠错、遮挡提示和安全提示拥有不同优先级与冷却时间：高优先级提示可以打断当前播报，重复纠错不会连续轰炸，普通鼓励也不会在队列中堆积。浏览器没有可用语音能力时，训练画面仍会显示文字提示。
